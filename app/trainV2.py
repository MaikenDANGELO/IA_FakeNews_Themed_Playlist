...
def clean_text(text):
    text = str(text).lower()  # Convertit en minuscule pour normaliser
    text = re.sub(r'https?://\S+|www\.\S+', '', text)  # Supprime URLs
    text = re.sub(r'<.*?>', '', text)  # Supprime balises HTML
    text = re.sub(r'\s+', ' ', text).strip()  # Nettoie espaces multiples et strip
    return text
...
try:
    df_b_train = pd.read_csv('hf://datasets/BeardedJohn/FakeNews/train.csv')
    df_b_val = pd.read_csv('hf://datasets/BeardedJohn/FakeNews/validation.csv')
    df_b_test = pd.read_csv('hf://datasets/BeardedJohn/FakeNews/test.csv')
    
    df_bearded = pd.concat([df_b_train, df_b_test, df_b_val])
    df_bearded['label'] = df_bearded['label'].map({0: 1, 1: 0}) # inverse les 0 en 1, et les 1 en 0
    data_frames.append(df_bearded[['text', 'label']])
...
EXPERIMENT_NAME = 'v1_best_finetunning'
SAVE_MODEL_DIR = f"./model_{EXPERIMENT_NAME}"
METRICS_FILE = f"./results_{EXPERIMENT_NAME}/metrics_report.json"
...
# EXPERIMENT_NAME = 'v2_bad_finetunning'
# SAVE_MODEL_DIR = f"./model_{EXPERIMENT_NAME}"
# METRICS_FILE = f"./results_{EXPERIMENT_NAME}/metrics_report.json"
# training_args = TrainingArguments(
#     output_dir=SAVE_MODEL_DIR,
...
# EXPERIMENT_NAME = "v3_fast_finetunning"
# SAVE_MODEL_DIR = f"./model_{EXPERIMENT_NAME}"
# METRICS_FILE = f"./results_{EXPERIMENT_NAME}/metrics_report.json"
# training_args = TrainingArguments(
#     output_dir=SAVE_MODEL_DIR,
...
# EXPERIMENT_NAME = 'v4_long_finetunning'
# SAVE_MODEL_DIR = f"./model_{EXPERIMENT_NAME}"
# METRICS_FILE = f"./results_{EXPERIMENT_NAME}/metrics_report.json"
# training_args = TrainingArguments(
#     output_dir=SAVE_MODEL_DIR,
